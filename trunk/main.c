...
void extractPhoto(FILE *input, FILE *output, int altoTile, int anchoTile); // It needs height and width for extracting thumbnails or full size
...
// Set's environment acording to photo size
	if(!thumbnailflag){
...
// Extracts photos
	...
	for(int i=0; i< CANT_PICS; i++, fseek(input,step,SEEK_CUR)){
		...
		extractPhoto(input,output,height,width);
...
void extractPhoto(FILE *input, FILE *output, int altoTile, int anchoTile){
	int colors[] = {3,1,2,0};
	int ancho, alto, inAncho, inAlto, y, x;
	int bmpBuff[altoTile*PIXELES][anchoTile*PIXELES];	// This stores the pic data before it get's printed to the file
	char buff[2];
		...
		for(ancho=0; ancho < anchoTile; ancho++)						// Height en tiles
			for(inAlto=0; inAlto < PIXELES; inAlto++){					// Each pixel coordinate Y
				...
				for(inAncho=0; inAncho < PIXELES ; inAncho++, z>>=1){	// Each pixel coordinate X
					int col = ((*buff & z) == z) * 2 + ((*(buff + 1) & z) == z);
					x = inAncho+(ancho*PIXELES);
					y = inAlto+(alto*PIXELES);
					
					bmpBuff[y][x] = col;	// Writes the pixel into the buffer
				}
			}
...
// Writes the buffer into the file
	...
	for(y = col; y >= 0; y--){
		for(x = 0; x < anchoTile*PIXELES; x+=2){
			// Make 1 byte from 2px
			int byte = colors[bmpBuff[y][x]] << 4 | colors[bmpBuff[y][x+1]];	
			fputc(byte,output);
		}
	}
}
